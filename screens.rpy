...
screen quick_menu():
    ...
    if quick_menu_circular:
        fixed:
            ...
            add "#00000080":  # Полупрозрачный черный фон
                size (400, 400)
                xcenter 0.5
                ycenter 0.5
            ...
            $ button_count = 6
            # Угол между кнопками
            $ angle_step = 360 / button_count
            
            # Кнопки кругового меню
            for i, item in enumerate([
                ("Игровой магазин", "store"),
                ("Аккаунт", "account"),
                ...
                $ angle = i * angle_step
                $ x_pos = radius * cos(angle * 3.14159 / 180)
                $ y_pos = radius * sin(angle * 3.14159 / 180)
                
                # Создаем кнопку с возможностью добавления изображения
                button:
                    # Позиционируем кнопку
                    xcenter 0.5 + x_pos
                    ycenter 0.5 + y_pos
                    # Минимальный размер для кликабельной области
                    minimum (140, 40)
                    ...
                    action [
                        SetVariable("quick_menu_circular", False),  # Закрыть меню
                        Function(handle_circular_menu_action, action_name)  # Обработать действие
                    ]
...
init python:
    def handle_circular_menu_action(action_name):
        if action_name == "store":
            # Открыть магазин
            renpy.run(ShowMenu("store"))
        elif action_name == "account":
            # Открыть экран аккаунта
            renpy.run(ShowMenu("account"))
        elif action_name == "preferences":
            # Открыть настройки
            renpy.run(ShowMenu("preferences"))
        elif action_name == "notebook":
            # Открыть блокнот
            renpy.run(ShowMenu("notebook"))
        elif action_name == "history":
            # Открыть историю
            renpy.run(ShowMenu("history"))
        elif action_name == "achievements":
            # Открыть достижения
            renpy.run(ShowMenu("achievements"))
    
    # Флаг видимости кругового меню
    quick_menu_circular = False
